...
func JWTAuth(jwtMgr *authn.JWTManager) gin.HandlerFunc {
	return JWTAuthWithRole(jwtMgr, "user")
}

// JWTAuthWithRole 在基础 JWT 校验上额外校验账号角色。
// 这样普通用户与管理员可以共用 JWT secret，但仍能通过 role 完成路由隔离。
func JWTAuthWithRole(jwtMgr *authn.JWTManager, expectedRole string) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if expectedRole != "" && claims.Role != expectedRole {
			c.AbortWithStatusJSON(http.StatusForbidden, gin.H{
				"code": "PERMISSION_DENIED", "message": "role not allowed",
			})
			return
		}
