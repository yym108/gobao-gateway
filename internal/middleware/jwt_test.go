...
func setupRouter(jwtMgr *authn.JWTManager) *gin.Engine {
	...
	r.GET("/protected", JWTAuth(jwtMgr), func(c *gin.Context) {
		c.JSON(200, gin.H{"user_id": c.GetInt64("userID"), "email": c.GetString("userEmail"), "role": c.GetString("userRole")})
	})
	return r
}
...
func TestJWTAuthWithRole_RejectsWrongRole(t *testing.T) {
	mgr := authn.NewJWTManager("test-secret", time.Hour)
	token, _, err := mgr.SignWithRole(1, "admin@test.com", "admin")
	require.NoError(t, err)

	r := gin.New()
	r.GET("/protected", JWTAuthWithRole(mgr, "user"), func(c *gin.Context) {
	...
	assert.Contains(t, w.Body.String(), "role not allowed")
}
