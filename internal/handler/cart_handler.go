...
type CartItem struct {
	...
	OptionSummary     string `json:"option_summary"`     // 当前条目的版本规格摘要，用于前端展示与分条存储
	Status            int32  `json:"status"`             // 当前商品状态：1=上架 2=下架
	Available         bool   `json:"available"`          // 当前条目是否允许继续购买/结算
	UnavailableReason string `json:"unavailable_reason"` // 当前不可购买原因，供前端直接提示
}
...
type addCartItemRequest struct {
	ProductID int64 `json:"product_id"` // 独立商品 ID，后端据此读取价格和规格快照
...
type CartHandler struct {
	...
	productClient productDetailReader // 商品详情读取接口，用于将 product_id 解析为后端权威快照
}
...
func (h *CartHandler) GetCart(c *gin.Context) {
	...
	if h.productClient != nil {
		resp = h.refreshCartAvailability(c.Request.Context(), resp)
	}
...
func (h *CartHandler) AddItem(c *gin.Context) {
	...
	cartItem, err := h.buildCartItemFromProductID(c.Request.Context(), req.ProductID, req.Quantity)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "非法购物车商品参数"})
		return
	}
...
// buildCartItemFromProductID 根据独立商品 ID 查询后端权威快照并构造购物车条目。
// 当前购物车只认独立商品本身，价格、图片与规格摘要全部以后端商品详情返回为准。
func (h *CartHandler) buildCartItemFromProductID(ctx context.Context, productID int64, quantity int32) (CartItem, error) {
	if h.productClient == nil {
	...
	for _, item := range detail.Variants {
		if item.ID != productID {
			continue
		}
		return CartItem{
			ProductID:         item.ID,
			Name:              resolveCartItemName(detail, detail.Product.Name),
			Price:             item.Price,
			Quantity:          quantity,
			ImageURL:          item.ImageURL,
			OptionSummary:     strings.TrimSpace(item.SpecLabel),
			Status:            item.Status,
			Available:         item.Status == 1 && item.StockQuantity > 0,
			UnavailableReason: resolveCartItemUnavailableReason(item.Status, item.StockQuantity),
		}, nil
	}

	product := detail.Product
	if product.ID <= 0 || product.ID != productID {
		return CartItem{}, errors.New("product not found")
	}
	return CartItem{
		ProductID:         product.ID,
		Name:              resolveCartItemName(detail, product.Name),
		Price:             product.Price,
		Quantity:          quantity,
		ImageURL:          product.ImageURL,
		OptionSummary:     strings.TrimSpace(product.SpecLabel),
		Status:            product.Status,
		Available:         product.Status == 1 && product.StockQuantity > 0,
		UnavailableReason: resolveCartItemUnavailableReason(product.Status, product.StockQuantity),
	}, nil
}

// resolveCartItemName 返回购物车条目展示名称。
// 优先使用独立商品名；商品名缺失时再回退到商品组名，避免写入空名称。
func resolveCartItemName(detail *client.ProductDetailDTO, fallback string) string {
	name := strings.TrimSpace(fallback)
	if name != "" {
		return name
	}
	return strings.TrimSpace(detail.Group.Name)
}

// refreshCartAvailability 重新读取购物车内各商品的后端权威状态，
// 避免商品被后台下架后，前端仍只能看到加入当时的旧快照。
func (h *CartHandler) refreshCartAvailability(ctx context.Context, resp CartResponse) CartResponse {
	for index := range resp.Items {
		item := resp.Items[index]
		detail, err := h.productClient.GetProductDetail(ctx, item.ProductID)
		if err != nil {
			resp.Items[index].Available = false
			resp.Items[index].UnavailableReason = "当前商品信息同步失败，请稍后重试"
			continue
		}

		refreshed, ok := resolveCartItemSnapshot(detail, item.ProductID, item.Quantity)
		if !ok {
			resp.Items[index].Available = false
			resp.Items[index].UnavailableReason = "当前商品不存在或已下线"
			continue
		}

		item.Status = refreshed.Status
		item.Available = refreshed.Available
		item.UnavailableReason = refreshed.UnavailableReason
		item.Price = refreshed.Price
		item.ImageURL = refreshed.ImageURL
		item.OptionSummary = refreshed.OptionSummary
		resp.Items[index] = item
	}
	return resp
}

// resolveCartItemSnapshot 从商品详情中提取指定独立商品的最新购物车快照。
func resolveCartItemSnapshot(detail *client.ProductDetailDTO, productID int64, quantity int32) (CartItem, bool) {
	for _, item := range detail.Variants {
		if item.ID != productID {
			continue
		}
...
// resolveCartItemUnavailableReason 将商品状态映射为购物车页可直接展示的用户提示。
func resolveCartItemUnavailableReason(status int32, stockQuantity int32) string {
	if status != 1 {
		return "当前商品已下架，暂时无法购买"
	}
	if stockQuantity <= 0 {
		return "当前商品暂时缺货"
	}
