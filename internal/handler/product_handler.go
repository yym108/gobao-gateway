...
// AdminUpdateStockRequest 是后台库存直接设置请求。
// Gateway 只负责参数承接与转发，库存并发控制仍由 Product 服务负责。
...
// ListProductGroups 处理 GET /api/v1/product-groups。
// 支持按 category_id 过滤，以及 page/page_size 分页参数。
func (h *ProductHandler) ListProductGroups(c *gin.Context) {
	...
	resp, err := h.client.ListProductGroups(c.Request.Context(), &productv1.ListProductGroupsRequest{
		CategoryId: catID,
		Page:       int32(page),
		PageSize:   int32(pageSize),
	})
...
// CreateProductGroup 处理 POST /api/v1/admin/product-groups。
// 请求体按 proto CreateProductGroupRequest 绑定，成功返回 201。
func (h *ProductHandler) CreateProductGroup(c *gin.Context) {
	var req productv1.CreateProductGroupRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}

	resp, err := h.client.CreateProductGroup(c.Request.Context(), &req)
	if h.handleGRPC(c, err) {
		return
	}
	c.JSON(http.StatusCreated, resp)
...
// UpdateProductGroup 处理 PUT /api/v1/admin/product-groups/:id。
// 路径参数中的商品组 ID 会覆盖请求体中的 ID。
func (h *ProductHandler) UpdateProductGroup(c *gin.Context) {
	id, _ := strconv.ParseInt(c.Param("id"), 10, 64)

	var req productv1.UpdateProductGroupRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
	...
	resp, err := h.client.UpdateProductGroup(c.Request.Context(), &req)
	if h.handleGRPC(c, err) {
		return
	}
	c.JSON(http.StatusOK, resp)
...
// DeleteProductGroup 处理 DELETE /api/v1/admin/product-groups/:id。
// 删除成功返回 204 No Content。
func (h *ProductHandler) DeleteProductGroup(c *gin.Context) {
	id, _ := strconv.ParseInt(c.Param("id"), 10, 64)
	_, err := h.client.DeleteProductGroup(c.Request.Context(), &productv1.DeleteProductGroupRequest{Id: id})
	if h.handleGRPC(c, err) {
		return
	}
	c.Status(http.StatusNoContent)
...
// UpdateProductStock 处理 PUT /api/v1/admin/products/:id/stock。
// 当前为后台独立商品库存直接设置入口，使用 expected_version 走 CAS 防并发覆盖。
func (h *ProductHandler) UpdateProductStock(c *gin.Context) {
	...
	resp, err := h.client.UpdateStock(c.Request.Context(), &productv1.UpdateStockRequest{
		ProductId:       id,
...
// 删除成功返回 200，并在存在商品组时附带提示消息。
func (h *ProductHandler) DeleteCategory(c *gin.Context) {
	id, _ := strconv.ParseInt(c.Param("id"), 10, 64)
	groupRes, err := h.client.ListProductGroups(c.Request.Context(), &productv1.ListProductGroupsRequest{
		CategoryId: id,
		Page:       1,
		PageSize:   1,
	})
